...
public class Client {
    ...
    final static int getPingInterval() {
        return 60000; //PING_INTERVAL_NAME, DEFAULT_PING_INTERVAL 1分钟
    }
    ...
    private class Call {
        int id;                                       // call id
        Writable param;                               // parameter  调用远程方法参数
        Writable value;                               // value, null if error   远程方法返回的结果，如果为空发生错误
        IOException error;                            // exception, null if value   远程调用抛出异常，存放异常
        boolean done;                                 // true when call is done    标识远程调用是否完成
        ...
        private synchronized void setupIOstreams() throws InterruptedException {
            ...
            try {
                ...
                while (true) {
                    ...
                    /**
                     * 写rpc header到server,代表rpc开始，"hrpc"+版本号
                     */
